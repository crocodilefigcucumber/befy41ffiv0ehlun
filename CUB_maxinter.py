...
RESULTS_CSV = "results/CUB/test_maxinter.csv"


with open(RESULTS_CSV, mode="w", newline="") as file:
    writer = csv.writer(file)
    writer.writerow(["model_type", "max_interventions", "test_acc"])
...
if __name__ == "__main__":
    ...
    NETWORKS = [
        network[len("maxiter_") + 1 :] for network in NETWORKS if "maxinter_" in network
    ]
    ...
    for network in NETWORKS:
        ...
        results = f"{REALIGNMENT_PATH}/maxinter_{network}/results.csv"
        df = pd.read_csv(results)

        for run_idx in df["run_idx"]:
            try:
                with open(
                    f"{REALIGNMENT_PATH}/maxinter_{network}/run_{run_idx}_config.json",
                    "r",
                ) as file:
            ...
            if network != "Baseline":
                state_dict = torch.load(
                    f"{REALIGNMENT_PATH}/maxinter_{network}/run_{run_idx}_best_model.pth",
                    map_location=device,
                )
            ...
            with open(RESULTS_CSV, mode="a", newline="") as file:  # open in append mode
                writer = csv.writer(file)
                writer.writerow([model_type, config["max_interventions"], test_acc])
